...
def graph_loss(V_pred,V_target):
    return trivariate_loss(V_pred,V_target)
...
def train(epoch):
    ...
    for cnt,batch in enumerate(loader_train): 
        ...
        y_abs = pred_traj_gt.permute(0,1,3,2).squeeze()
        num_peds = y_abs.shape[0]
        dist_ = torch.zeros(num_peds,args.pred_seq_len,1).cuda()
        for i in range(args.pred_seq_len):
            dist_[:,i,:] = dist(y_abs[:,-1,:], y_abs[:,i,:]).unsqueeze(1)
        
        V_tr = V_tr.squeeze()
        A_tr = A_tr.squeeze()
        V_pred = V_pred.squeeze()

        V_tr = torch.cat([V_tr, dist_.permute(1,0,2)], dim=-1) 
